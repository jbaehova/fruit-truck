...
import { Check, ExternalLink, Pencil, ShieldCheck, SlidersHorizontal, X } from "lucide-react";
              ...
              <section className="settings-credential" aria-labelledby="settings-api-key-title">
                ...
                {status === null ? (
                  ...
                  <div className="credential-summary">
                    ...
                    <span><strong>{t("apiKeyConfigured")}</strong><small>{status.maskedKey}</small></span>
                ...
                <div className="credential-location">
                  <ShieldCheck />
                  <span><strong>{t("localPlaintextStorage")}</strong><small>{status?.path ?? "~/.fruit-truck/credentials.json"}<br />{t("storagePermissions")}</small></span>
                </div>
                <p className="settings-note">{t("keyPrivacyHint")}</p>
                <Button variant="ghost" size="sm" className="external-link" nativeButton={false} render={<a href="https://openrouter.ai/settings/keys" target="_blank" rel="noreferrer" />}>
