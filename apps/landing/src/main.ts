...
const DOWNLOAD_URL = "https://github.com/jbaehova/fruit-truck/releases/latest/download/Fruit-Truck-macOS-universal.dmg";
const STORAGE_KEY = "fruit-truck-language";

const copy: Record<Locale, Copy> = {
  en: {
    metaTitle: "Fruit Truck — Make every model fit",
    ...
    workBody: "Pick a model. Fruit Truck reads its capabilities live, keeps unsupported fields out, and shows the exact JSON before anything leaves your Mac.",
    ...
    installTitle: "HOW TO INSTALL.", installBody: "Fruit Truck is distributed without Apple notarization. macOS will ask you to approve it once.",
    installOne: "Open the DMG and drag Fruit Truck to Applications.", installTwo: "Try to open the app once.", installThree: "Open System Settings → Privacy & Security → Open Anyway.",
    ...
    openAnywayAlt: "macOS Security settings showing the Open Anyway button for Fruit Truck",
    updateLabel: "IN-APP UPDATES FROM 0.2.0", updateTitle: "UPDATE IN APP. KEEP YOUR KEY.", updateBody: "Install 0.2.0 once, then Fruit Truck checks for signed updates whenever it starts.",
    ...
    updatePersistence: "Your API key stays in ~/.fruit-truck, outside the app bundle, so updating does not remove it.", downloadLatest: "DOWNLOAD THE LATEST DMG",
    finalTitle: "MAKE SOMETHING MOVE.", footerNote: "Open source. Built for OpenRouter. macOS only.",
  },
  ko: {
    metaTitle: "Fruit Truck — 모든 모델을 아이디어에 맞게",
    ...
    workBody: "모델을 고르면 Fruit Truck이 기능을 실시간으로 읽습니다. 지원하지 않는 필드는 빼고, Mac을 떠나기 전 정확한 JSON을 보여줍니다.",
    ...
    installTitle: "설치 방법", installBody: "Fruit Truck은 Apple 공증 없이 배포됩니다. macOS에서 최초 한 번만 허용하면 됩니다.",
    installOne: "DMG를 열고 Fruit Truck을 Applications로 드래그합니다.", installTwo: "앱을 한 번 실행해 봅니다.", installThree: "시스템 설정 → 개인정보 보호 및 보안 → 그래도 열기를 선택합니다.",
    ...
    openAnywayAlt: "Fruit Truck의 그래도 열기 버튼이 표시된 macOS 보안 설정",
    ...
    updatePersistence: "API 키는 앱 밖 ~/.fruit-truck에 저장되므로 업데이트해도 유지됩니다.", downloadLatest: "최신 DMG 다운로드",
